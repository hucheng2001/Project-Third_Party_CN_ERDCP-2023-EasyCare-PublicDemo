...
import 'nearEvent.dart';

// 列表聚合上拉加载下拉刷新

class MyNearEventList extends StatefulWidget {
  // 传入参数
  const MyNearEventList({
    Key? key,
  ...
  @override
  State<StatefulWidget> createState() => MyNearEventListState();
}

// 构建
class MyNearEventListState extends State<MyNearEventList> {
  // 上拉加载回调函数
  ...
  _MakeList() {
    ...
    theList = [
      ...
      child:const Align(
          ...
          child: Text('Event Tip：',style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600),),
        )
      )
    ];
    for(var item in widget.DataForm) {
      theList.add(MyNearEvent(
        onDel:(){widget.onDel(item);},
        data:item,
        onEdit: (){widget.onEdit(item);},
      ));
    }
    theList.add(const SizedBox(height: 50,));
    return theList;
  }
